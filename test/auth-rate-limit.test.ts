 ...
 * vi.resetModules() re-evaluates the module, which re-creates the module-level
 * Map inside createMemoryFixedWindowRateLimiter.
 */
...
describe("isAuthSensitivePath", () => {
  ...
  it("returns false for /api/auth/csrf (CSRF token fetch, not an attack surface)", () => {
...
describe("checkAuthRateLimit — IP isolation", () => {
  it("counts are tracked independently per IP address", async () => {
    ...
    // A completely different IP must start with a clean slate
...
describe("checkAuthRateLimit — reset timestamp", () => {
  ...
  it("reset points to the end of the current 15-minute fixed window", async () => {
    ...
    // Window starts at epoch 0 and ends AUTH_WINDOW_MS ms later.
